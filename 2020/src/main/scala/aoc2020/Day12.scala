...
object Day12 extends AoC:
  ...
  case class Ship(x: Int = 0, y: Int = 0, face: Char = 'E'):
    ...
    private final def turn(current: Char, direction: Char, by: Int): Char =
      if (by == 0) current else (current, direction).runtimeChecked match
  ...
  case class WayPoint(x: Int, y: Int):
    ...
    @tailrec
    final def turn(current: Char, by: Int, x0: Int = x, y0: Int = y): WayPoint =
      if by == 0 then copy(x = x0, y = y0) else current.runtimeChecked match
