...
object Day15 extends AoC:
  ...
  object Ingredient:
    ...
    def fromString(line: String): Ingredient =
      line match
  ...
  case class Recipe(measurements: Map[Ingredient, Int]):
    ...
    def score: Int =
      ...
      def scores(ingredient: Ingredient, spoons: Int): Vector[Int] =
        Vector( ingredient.capacity   * spoons
      ...
      measurements
        .map(scores)
        .fold(Vector.empty[Int]): (total,current) =>
          if total.isEmpty then current else total.zip(current).map(_ + _)
        .map: score =>
          if score <= 0 then 0 else score
        .product

    def calories: Int =
      measurements.map((ingredient, spoons) => ingredient.calories * spoons).sum



  val ingredients: Vector[Ingredient] =
    lines.map(Ingredient.fromString)
