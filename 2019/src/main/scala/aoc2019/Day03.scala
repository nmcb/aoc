...
object Day03 extends AoC:
  ...
  def append(wire: Vector[Pos], cmd: String): Vector[Pos] =
    ...
    cmd.head match
      case 'R' => wire ++ (1 to length).map(i => (next.x + i, next.y))
      case 'L' => wire ++ (1 to length).map(i => (next.x - i, next.y))
      case 'U' => wire ++ (1 to length).map(i => (next.x, next.y + i))
      case 'D' => wire ++ (1 to length).map(i => (next.x, next.y - i))
