...
object Day07 extends AoC:
  ...
  val nodes: Nodes =
    lines
      .collect:
        case s"$name ($weight) -> $children" => name -> Node(name, weight.toInt, children.split(", ").toVector)
        case s"$name ($weight)"              => name -> Node(name, weight.toInt, Vector.empty)
      .toMap
