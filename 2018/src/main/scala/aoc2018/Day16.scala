...
object Day16 extends AoC:
  ...
  enum Inst derives CanEqual:
    ...
    def execute(codes: Codes)(mem: Mem): Mem =
      val (_, a, b, c) = codes
      this match
        case ADDR => mem.setRR(a, b, c, _ + _)
        case ADDI => mem.setRI(a, b, c, _ + _)
        case MULR => mem.setRR(a, b, c, _ * _)
        case MULI => mem.setRI(a, b, c, _ * _)
        case BANR => mem.setRR(a, b, c, _ & _)
        case BANI => mem.setRI(a, b, c, _ & _)
        case BORR => mem.setRR(a, b, c, _ | _)
        case BORI => mem.setRI(a, b, c, _ | _)
        case SETR => mem.setRN(a, c)
        case SETI => mem.setIN(a, c)
        case GTIR => mem.setIR(a, b, c, (a, b) => if a  > b then 1 else 0)
        case GTRI => mem.setRI(a, b, c, (a, b) => if a  > b then 1 else 0)
        case GTRR => mem.setRR(a, b, c, (a, b) => if a  > b then 1 else 0)
        case EQIR => mem.setIR(a, b, c, (a, b) => if a == b then 1 else 0)
        case EQRI => mem.setRI(a, b, c, (a, b) => if a == b then 1 else 0)
        case EQRR => mem.setRR(a, b, c, (a, b) => if a == b then 1 else 0)
  ...
  val (tests: Vector[Test], program: Vector[Codes]) =
    val Array(prefix, postfix) = input.split("\n\n\n")
